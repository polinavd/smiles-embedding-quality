...
Each run:
  ...
  3. Keeps the checkpoint by default -- later work (GLUE classification probes,
     RankMe and other geometric metrics) needs the actual weights, not just the
     per-run metrics. Pass --delete-checkpoints to discard them after scoring
     (e.g. BERT-base checkpoints are ~400MB each, so a large grid can add up).
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        "since later work (GLUE probes, RankMe and other geometric metrics) needs the weights.",
    )
    args = parser.parse_args()
    args.keep_checkpoints = not args.delete_checkpoints
